...
- **Gmail SMTP alerts don't send from Railway.** Gmail SMTP (both port 587 and 465) times out from Railway's network — several PaaS providers block outbound SMTP by default to prevent spam relaying. Production deployments must set `RESEND_API_KEY` or `BREVO_API_KEY` (an HTTP-API email provider) rather than relying on raw SMTP.
- **Resend's free tier only delivers to the account owner's own email until a domain is verified.** Without a verified sending domain (`resend.com/domains`, needs a domain you control), Resend silently rejects alerts to any recipient other than the Resend account's own signup address — meaning other users' configured alert emails never arrive. `BREVO_API_KEY` doesn't have this restriction: Brevo only needs a single verified sender *email address* (no domain/DNS), and delivers to any recipient once that's done. See `backend/.env.example`.
